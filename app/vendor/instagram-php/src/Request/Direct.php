...
class Direct extends RequestCollection
{
        ...
        $prefetch = false)
    {
        ...
        if ($cursorId !== null) {
            $request->addParam('cursor', (string)$cursorId);
        }
        if ($prefetch) {
        ...
        array $options = [])
    {
        ...
        $internalMetadata->setPhotoDetails(Constants::FEED_DIRECT, $photoFilename);

        $response = $this->ig->internal->uploadPhotoData(Constants::FEED_DIRECT, $internalMetadata);
